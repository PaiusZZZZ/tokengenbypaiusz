...
async function main() {
  ...
  if (choice == 1) {
    ...
    if (choicewbwile == "y") {
      webhook = prompt("Press Enter to Continue");
    }
    console.log(
      "[" +
        "!".green +
        "] Do you want to send the tokens to the Server (Premium Feature Contact via Discord): "
    );
    let choicewbwile2 = prompt("[?]>");
    if (choicewbwile2 == "y") {
      sunucudavet = prompt("Coded by PaiusZ ");
    }
    let tokensname = prompt(
      "Token Names: ( Fill If You Want Or Don't And Get Random ) "
    );
    let HowTokens = prompt("How Many Tokens Do You Want To Create: ");
    ...
    async function create_accinfos(chrome, disc) {
      ...
      const password = "paiusz123"; // THIS WILL BE THE PASSWORDS OF ALL THE TOKENS YOU CREATE
